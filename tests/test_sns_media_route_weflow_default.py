...
class TestSnsMediaRouteWeFlowDefault(unittest.TestCase):
    ...
    def test_route_logs_redacted_identity_and_local_match_details(self):
        ...
        self.assertIn('"result": "local-cache"', logs)
        self.assertNotIn('"matchedBy":', logs)
        self.assertNotIn('"candidatePath":', logs)
        self.assertNotIn("matched-local.jpg", logs)
        self.assertNotIn("post-42", logs)
        self.assertNotIn("media-7", logs)
        self.assertNotIn("mmsns.qpic.cn", logs)
        self.assertNotIn('"urlIdentity":', logs)
        self.assertNotIn('"tokenHash":', logs)
        self.assertNotIn('"keyHash":', logs)
        self.assertNotIn('"responseSha256":', logs)
        ...
        with TemporaryDirectory() as td:
            ...
            for expected_match, key_results, md5, md5_result in cases:
                with self.subTest(expected_match=expected_match):
                    ...
                    logs = self._render_log_calls(log_info)
                    self.assertEqual(resp.status_code, 200)
                    self.assertIn(f"sns.media {expected_match}:probe", logs)
                    self.assertNotIn('"matchedBy":', logs)
