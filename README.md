...
常规版聚焦于**解密、读取、导出与年度总结**等只读能力；**高级版**提供 **61 项能力**，覆盖消息修改、消息补录、微信动作、朋友圈同步与互动、群聊和联系人管理，以及自动化任务。当前公开版本仅展示功能说明与示例动画，不包含高级功能的执行实现；实际使用需要匹配微信版本的高级版。
...
演示使用虚构数据，不连接微信。真实查找、发送和互动需要登录微信并通过微信联网；好友申请是否收到、是否通过与最终执行结果分别确认。自动化配置默认关闭；目录筛选、滚动分页及账号工作区属于配套能力，不重复计数。
