...
其中**消息修改、消息补录以及会话标记已读、会话免打扰共 27 项只写入本地解密副本**，单向、不回写微信，对方看不到；其余 34 项经微信客户端执行或读取（其中联系人变化记录只比对本机快照）。演示动画使用虚构数据，不连接微信；真实查找、发送与互动需要登录微信并通过微信联网，好友申请是否送达、是否通过与最终执行结果分别确认。自动化任务默认关闭，需自行配置并手动启动。
