/* ════════════════════════════════════════════════════════════
   ...
   每项字段（edge 可选，local 自动算出）：
     ...
     flow    这件事串成几步（3 步；本地存档类是「我怎么整理」，真实动作类是「工作流怎么跑」）——
             官网首屏动画下方现在只显示它 + 场景标签 + 边界
     edge    边界（可选；缺省按分组取 main.js 里的 SCENE_EDGE）
     local   是否只动本地解密副本（文件末尾按分组自动补上，勿手写）
   改一处四处同步。key 是场景文件的索引，别随手改。

   ⚠️ 两类能力的世界观不通用（见 README「场景优先」）：
   - 本地存档类 27 项（消息修改 8 + 消息补录 17 + 标记已读 + 免打扰）：只写本地解密副本，
     单向、不回写微信。语境是私人的——一气之下清空了和 TA 的聊天记录，事后一条条找回。
...
export const PRO_GROUPS = [
  {
    ...
    items: [
      { key: "edit-text", name: "修改文字消息", caption: "已发出的文字，原地改成你要的样子，直接落库。", use: "按截图核对", need: "恢复回来的那句话和截图对不上，改成原话",
        story: "备份里捞回来的那句话，和手机里存的截图差了两个字——按截图把它改回原话。", flow: ["翻出当时的截图", "把这句改回原话", "写回本地副本"] },
      { key: "edit-source", name: "编辑消息源码", caption: "打开消息的 XML 源码，逐字段手改。", use: "卡片修复", need: "恢复回来的链接卡片标题乱码，改源码让它可读",
        story: "恢复回来的链接卡片，标题成了一串乱码，打开 XML 逐字段改，卡片才重新读得出来。", flow: ["打开消息源码", "逐字段改回可读", "存回本地副本"] },
      ...
      { key: "edit-restore", name: "恢复原消息", caption: "改过头了？一键恢复到原始消息。", use: "一键回退", need: "改过头了，把原来那句原样还回来",
        story: "改过头了，一句本来没错的话被自己改乱——原值还留着，一键还回去。", flow: ["选中改过的消息", "取回原值", "还原本地副本"] },
...
// 只写本地解密副本、不回写微信的 27 项：消息修改 8 + 消息补录 17 + 这两项会话状态
const LOCAL_ONLY = new Set(["chat-mark-read", "chat-set-mute"]);

// 每项就地补上所属分组、全局序号（1 起）与「是否只动本地副本」；PRO_ITEMS 与分组里的是同一批对象
let _n = 0;
...
// 只动本地解密副本的那批（27 项）：写场景、写文案、做校验时用它对边界
