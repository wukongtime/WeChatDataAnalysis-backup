  ...
  <details class="agent-tool" :class="[`is-${status}`, { 'is-grouped': grouped }]" :open="expanded" @toggle="expanded = $event.target.open">
    ...
    <div class="agent-tool-detail" :class="{ 'agent-tool-timeline': grouped }">
      <section v-for="(item, index) in items" :key="item.id" class="agent-tool-attempt">
        <component :is="grouped ? 'details' : 'div'" class="agent-tool-inspection" :open="grouped && viewState?.[`tool-detail:${item.id}`]" @toggle="viewState && (viewState[`tool-detail:${item.id}`] = $event.target.open)">
        <summary v-if="grouped" class="agent-tool-attempt-row" title="展开本次读取详情">
          ...
          <strong>{{ item.action === 'compact_context' ? `第 ${index + 1} 次整理` : item.cached ? '复用已读结果' : index === 0 ? '首次读取' : `第 ${index + 1} 次读取` }}</strong>
        ...
        <p v-else-if="item.result">{{ item.cached ? '复用已读结果 · ' : '' }}{{ item.result.returned ?? 0 }} 条结果{{ item.result.has_more ? ' · 还有更多' : '' }}</p>
...
const status = computed(() => ['running', 'failed', 'paused', 'cancelled', 'interrupted', 'superseded'].find(status => props.items.some(item => item.status === status)) || first.value.status)
