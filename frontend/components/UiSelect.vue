<template>
  <span class="ui-select" :class="{ 'is-plain': plain, 'is-mono': mono }">
    ...
    <Teleport to="body">
      <div v-if="open" :id="listId" ref="menu" class="ui-select-menu" :class="{ 'is-mono': mono }"
        role="listbox" :aria-label="label" :aria-busy="loading" :style="position" @mousedown.prevent>
        ...
        <div v-for="(option, index) in options" :id="`${listId}-${index}`" :key="option.value"
        ...
        <div v-if="!options.length && !loading && !loadError" class="ui-select-empty">暂无可选项</div>
      </div>
    </Teleport>
  </span>
</template>
...
const close = () => { open.value = false; search = '' }
...
watch(() => props.options, (options, previous) => {
