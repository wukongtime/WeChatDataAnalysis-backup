<template>
  <div class="qa-toolbar">交互验收 · 虚构数据<button @click="dark=!dark">切换深浅色</button><button @click="narrow=!narrow">切换窄窗口</button></div>
  <main :class="{dark,narrow}">
    <aside><strong><i class="fa-solid fa-gear"></i> 设置</strong><span>桌面行为</span><span class="selected">AI 服务</span><span>语音转文字</span><span>MCP 接入</span><span>数据库与密钥</span><span>启动偏好</span><span>聊天与媒体</span></aside>
    <div class="qa-content"><header>AI 服务</header><div class="qa-scroll"><AiSettings /></div></div>
  </main>
</template>
<script setup>
