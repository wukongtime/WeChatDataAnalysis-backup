...
覆盖范围包括官方备份/版本校验、原子换位、调试 entitlement、LLDB 输出解析、PBKDF2 参数和 salt 过滤、内部返回点预检、候选数据库验真、状态文件权限、取消/超时/异常恢复、活动库优先和发布审计。

完整测试也在同一 Python 环境中对官方基线和本 Draft 分别执行。官方 `f6583c5` 基线为 `828 passed, 110 failed, 9 errors, 2 skipped`；本 Draft 为 `876 passed, 110 failed, 9 errors, 2 skipped`。新增 48 项全部通过，失败/错误数量与基线完全一致。现有失败集中于本机未提供仓库受限 native core、导出完整性运行时和语音模型环境，因此不把它们误报为本 PR 引入的回归。
...
- 当前真实记录不能推断 Intel Mac 或未来微信版本已兼容；
