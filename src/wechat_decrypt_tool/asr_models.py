"""新语音模型的固定版本、资产校验和能力声明；仅提供四个受支持的模型选项。"""
...
DEFAULT_VOICE_MODEL = "zipformer-small-ctc-int8"
RETIRED_VOICE_MODELS = frozenset({"tiny", "base", "small", "medium", "large-v3", "qwen3-asr-17b-hf"})
NEW_MODEL_CATALOG = (
    ...
    dict(id="qwen3-asr-06b-onnx-int4", name="Qwen3-ASR 0.6B · CPU", size="约 2.03 GB", speed="CPU",
         quality="质量优先", description="无需独立显卡，适合注重识别质量的中文转写；建议电脑配备 16 GB 内存。"),
    dict(id="qwen3-asr-06b-hf", name="Qwen3-ASR 0.6B · GPU", size="约 1.58 GB", speed="NVIDIA GPU",
         quality="质量优先", description="适合注重识别质量的中文转写，需 NVIDIA 显卡和 Qwen GPU 运行组件；速度慢于 Turbo。"),
)
